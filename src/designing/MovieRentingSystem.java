...
class MovieRentingSystem {
    ...
    public MovieRentingSystem(int n, int[][] entries) {
        ...
        for (int[] entry : entries) {
            ...
            trie.insert(List.of(shop, movie, price));
        }
    ...
    public void rent(int shop, int movie) {
        ...
        int price = trie.getPrice(List.of(shop, movie));
    ...
    // this will keep a tree of shop -> movie -> price
    // it will help us to get that in what price a shop keeps it particular movie
    static class Trie {
        ...
        void insert(List<Integer> list) {
            ...
            for (Integer val : list) {
                temp.children.putIfAbsent(val, new Node());
                temp = temp.children.get(val);
            }
        }

        Integer getPrice(List<Integer> list) {
            Node temp = root;
            for (Integer val : list) {
                if (!temp.children.containsKey(val)) {
                    return 0;
                }
                temp = temp.children.get(val);
            }

            Set<Integer> keys = temp.children.keySet();
            return !keys.isEmpty() ? keys.iterator().next() : 0;
        }
