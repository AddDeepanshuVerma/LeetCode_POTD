...
class MovieRentingSystem_chatGPT {
    ...
    public MovieRentingSystem_chatGPT(int n, int[][] entries) {
    ...
    public List<Integer> search(int movie) {
        TreeSet<Item> movies = unRentedMovies.get(movie);
        return movies == null ? Collections.emptyList() : movies.stream().limit(5).map(item -> item.shopId).toList();
    }

    public List<List<Integer>> report() {
        return rentedMovies.stream().limit(5).map(item -> List.of(item.shopId, item.movieId)).toList();
    }
